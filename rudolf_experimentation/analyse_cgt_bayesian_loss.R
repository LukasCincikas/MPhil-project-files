...
# - updated and edited by Lukas Cincikas, 12/2020-07/2021
...
options(warn = 1)  # Warnings become errors with 2; warnings remain warnings with 1.
...
STAN_GROUPS_CODE <- readr::read_file(file.path(
  THIS_DIR, "cgt_romeu2020_model12_combined_no_condition.stan"))
  ...
  max_treedepth = 10)
{
  ...
  cat("... made\n")
  #shinystan::launch_shinystan(shiny)
...
analyseMockDataAdvanced <- function(bayesian = TRUE, figures = TRUE)
{
  mock_data_2x2g <<- loadDataFromCsv(
    file.path(SYNTHETIC_DATA_DIR, "real_controls_comparison.csv"))
  mock_standata_2x2g <<- makeStanDataFromDataTable(mock_data_2x2g)
  if (bayesian) {
    mock_results_2x2g <<- analyseGroups(
      standata = mock_standata_2x2g, model_name = "controls_comparison_M12_combined")
    saveOutput(mock_results_2x2g$fit)
    # Specimen group comparison
    summary_of_mock_results_2x2g <<- stanfunc$annotated_parameters(
      fit = mock_results_2x2g$fit,
      probs = c(0.025, 0.975),
      par_regex = "^(?:overall_|placebo_|naltrexone_|drug_|experim_|control_)*(?:group|condition).*\\[*(?:1|1,2|2|3|4|1,3|1,4|2,3|2,4|3,4)*\\]*$"
    )
