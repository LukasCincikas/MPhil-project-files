...
parameters {
    ...
    //Condition means, in N(0,1) space:
    vector[N_CONDITIONS] nspace1_condition_mean_alpha;
    vector[N_CONDITIONS] nspace1_condition_mean_red_bias;
    vector[N_CONDITIONS] nspace1_condition_mean_gamma;
    vector[N_CONDITIONS] nspace1_condition_mean_rho;
    vector[N_CONDITIONS] nspace1_condition_mean_beta;
    ...
    vector<lower=0>[N_CONDITIONS] nspace1_condition_sd_alpha;
    vector<lower=0>[N_CONDITIONS] nspace1_condition_sd_red_bias;
    vector<lower=0>[N_CONDITIONS] nspace1_condition_sd_gamma;
    vector<lower=0>[N_CONDITIONS] nspace1_condition_sd_rho;
    vector<lower=0>[N_CONDITIONS] nspace1_condition_sd_beta;
    ...
    //subject-condition interactions (deviations from condition mean?):
    vector[N_SUBJECTS] nspace2_condition_effect_alpha;
    vector[N_SUBJECTS] nspace2_condition_effect_red_bias;
    vector[N_SUBJECTS] nspace2_condition_effect_gamma;
    vector[N_SUBJECTS] nspace2_condition_effect_rho;
    vector[N_SUBJECTS] nspace2_condition_effect_beta;
}
...
transformed parameters {
    ...
    for (s in 1:N_SUBJECTS) {
        ...
        int c = condition_num_by_subjects[s];
        int sgm = segment_num_by_subjects[s];
        ...
        // Bounded parameters: inverse probit-transformed to (0, 1) then scaled:
        nspace1_condition_effect_alpha =
            nspace2_condition_effect_alpha[s] * nspace1_condition_sd_alpha[c];
        nspace1_subject_effect_alpha =
            nspace2_subject_effect_alpha[sgm] * nspace1_group_sd_alpha[g];
        nspace1_subject_alpha =
            nspace1_group_mean_alpha[g] + nspace1_subject_effect_alpha + nspace1_condition_mean_alpha[c] + nspace1_condition_effect_alpha;
        subject_alpha[s] = Phi(nspace1_subject_alpha) * ALPHA_UPPER_LIMIT;

        nspace1_condition_effect_red_bias =
            nspace2_condition_effect_red_bias[s] * nspace1_condition_sd_red_bias[c];
        nspace1_subject_effect_red_bias =
            nspace2_subject_effect_red_bias[sgm] * nspace1_group_sd_red_bias[g];
        nspace1_subject_red_bias =
            nspace1_group_mean_red_bias[g] + nspace1_subject_effect_red_bias + nspace1_condition_mean_red_bias[c] + nspace1_condition_effect_red_bias;
        subject_red_bias[s] = Phi(nspace1_subject_red_bias);

        nspace1_condition_effect_gamma =
            nspace2_condition_effect_gamma[s] * nspace1_condition_sd_gamma[c];
        nspace1_subject_effect_gamma =
            nspace2_subject_effect_gamma[sgm] * nspace1_group_sd_gamma[g];
        nspace1_subject_gamma =
            nspace1_group_mean_gamma[g] + nspace1_subject_effect_gamma + nspace1_condition_mean_gamma[c] + nspace1_condition_effect_gamma;
        subject_gamma[s] = exp(nspace1_subject_gamma);

        // Unbounded parameters: exponentially transformed to (0, +inf):
        nspace1_condition_effect_rho =
            nspace2_condition_effect_rho[s] * nspace1_condition_sd_rho[c];
        nspace1_subject_effect_rho =
            nspace2_subject_effect_rho[sgm] * nspace1_group_sd_rho[g];
        nspace1_subject_rho =
            nspace1_group_mean_rho[g] + nspace1_subject_effect_rho + nspace1_condition_mean_rho[c] + nspace1_condition_effect_rho;
        subject_rho[s] = exp(nspace1_subject_rho);

        nspace1_condition_effect_beta =
            nspace2_condition_effect_beta[s] * nspace1_condition_sd_beta[c];
        nspace1_subject_effect_beta =
            nspace2_subject_effect_beta[sgm] * nspace1_group_sd_beta[g];
        nspace1_subject_beta =
            nspace1_group_mean_beta[g] + nspace1_subject_effect_beta + nspace1_condition_mean_beta[c] + nspace1_condition_effect_beta;
...
#include /include_model_declarations.stan
    ...
    sampleNormal_VRR_lp(nspace1_condition_mean_alpha,    0, 1);
    sampleNormal_VRR_lp(nspace1_condition_mean_red_bias, 0, 1);
    sampleNormal_VRR_lp(nspace1_condition_mean_gamma,    0, 1);
    sampleNormal_VRR_lp(nspace1_condition_mean_rho,      0, 1);
    sampleNormal_VRR_lp(nspace1_condition_mean_beta,     0, 1);
    ...
    sampleNormalLowerBound_VRR_lp(nspace1_condition_sd_alpha,    0, PRIOR_SD_FOR_GROUP_STANDARD_NORMAL, 0);
    sampleNormalLowerBound_VRR_lp(nspace1_condition_sd_red_bias, 0, PRIOR_SD_FOR_GROUP_STANDARD_NORMAL, 0);
    sampleNormalLowerBound_VRR_lp(nspace1_condition_sd_gamma,    0, PRIOR_SD_FOR_GROUP_STANDARD_NORMAL, 0);
    sampleNormalLowerBound_VRR_lp(nspace1_condition_sd_rho,      0, PRIOR_SD_FOR_GROUP_STANDARD_NORMAL, 0);
    sampleNormalLowerBound_VRR_lp(nspace1_condition_sd_beta,     0, PRIOR_SD_FOR_GROUP_STANDARD_NORMAL, 0);
    ...
    sampleNormal_VRR_lp(nspace2_condition_effect_alpha,    0, 1);
    sampleNormal_VRR_lp(nspace2_condition_effect_red_bias, 0, 1);
    sampleNormal_VRR_lp(nspace2_condition_effect_gamma,    0, 1);
    sampleNormal_VRR_lp(nspace2_condition_effect_rho,      0, 1);
    sampleNormal_VRR_lp(nspace2_condition_effect_beta,     0, 1);
...
generated quantities {
    ...
    matrix[N_GROUPS*N_CONDITIONS, N_GROUPS*N_CONDITIONS] group_sd_diff_nspace1_alpha;
    matrix[N_GROUPS*N_CONDITIONS, N_GROUPS*N_CONDITIONS] group_sd_diff_nspace1_red_bias;
    matrix[N_GROUPS*N_CONDITIONS, N_GROUPS*N_CONDITIONS] group_sd_diff_nspace1_gamma;
    matrix[N_GROUPS*N_CONDITIONS, N_GROUPS*N_CONDITIONS] group_sd_diff_nspace1_rho;
    matrix[N_GROUPS*N_CONDITIONS, N_GROUPS*N_CONDITIONS] group_sd_diff_nspace1_beta;

    // ========================================================================
    // Calculate
    // ========================================================================

    // Group x condition means, done as loops since it can't(?) be vectorized
    
    int tracker = 1;
    for (gr in 1:N_GROUPS) {
        for (con in 1:N_CONDITIONS) {
            group_mean_alpha[tracker] = Phi(nspace1_group_mean_alpha[gr] + nspace1_condition_mean_alpha[con]) * ALPHA_UPPER_LIMIT;
            group_mean_red_bias[tracker] = Phi(nspace1_group_mean_red_bias[gr] + nspace1_condition_mean_red_bias[con]);
            group_mean_gamma[tracker] = exp(nspace1_group_mean_gamma[gr] + nspace1_condition_mean_gamma[con]);
            group_mean_rho[tracker] = exp(nspace1_group_mean_rho[gr] + nspace1_condition_mean_rho[con]);
            group_mean_beta[tracker] = exp(nspace1_group_mean_beta[gr] + nspace1_condition_mean_beta[con]);
            tracker = tracker + 1;
        }
    }
