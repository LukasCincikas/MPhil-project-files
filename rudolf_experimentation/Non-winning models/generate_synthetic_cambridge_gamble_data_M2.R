...
# generate_synthetic_cambridge_gamble_data_M2.R
#
...
utilityFunction1 <- function(x, rho, delta)
{
  # Utility function log(1+x)
  stopifnot(rho >= 0)
  return((-1) * delta * (x ^ rho))
}

utilityFunction0 <- function(x, rho)
{
  ...
  debug = TRUE)
{
  ...
  utility_after_win <- utilityFunction0(x_capital_after_win, rho)
  utility_after_loss <- utilityFunction1(x_capital_after_win, rho, delta)
...
makeSpecimensWithinSubjects <- function(n_subjects_per_group = 25, seed = NULL)
{
  ...
  write.csv(
    data_holder,
    file = file.path(SYNTHETIC_DATA_DIR, "mock_data_rnc_2x2groups_M2.csv"),
    row.names = FALSE
  )
}
