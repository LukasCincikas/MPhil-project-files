...
# generate_synthetic_cambridge_gamble_data_M9.R
#
  ...
  debug = TRUE)
{
  ...
  utility_after_loss <- utilityFunction3(y_capital_after_loss, rho)
  
  unchosen_colour_utility <- 1 - colour_utility
  expected_utility <- colour_utility * utility_after_win + unchosen_colour_utility * utility_after_loss
  # ... [Romeu 2020], eq. 6
...
makeSingleRun <- function(alpha, red_bias, gamma, rho, beta)
{
  ...
  for (block in 1 : N_BLOCKS) {
    ...
    while (trial_within_block <= MAX_TRIALS_PER_BLOCK) {
      ...
      #What was the probability that the subject will choose the colour they chose?
      chosen_colour_utility <-
        if(chose_red[trial]) p_subject_chooses_red[trial] else 1 - p_subject_chooses_red[trial]
      
      # What are the expected utilities of each possible bet?
      penalized_expected_utility <- getPenalizedExpectedUtility(
        colour_utility = chosen_colour_utility,
...
makeSpecimens <- function(n_subjects_per_group = 50, seed = NULL)
{
  ...
  write.csv(
    ...
    file = file.path(SYNTHETIC_DATA_DIR, "mock_data_rnc_1subject_M9.csv"),
    row.names = FALSE
  )
  write.csv(
    rbind(
    ...
    file = file.path(SYNTHETIC_DATA_DIR, "mock_data_rnc_2subjects_M9.csv"),
    row.names = FALSE
  )
  write.csv(
    makeMultipleSubjectsSameParameters(
    ...
    file = file.path(SYNTHETIC_DATA_DIR, "mock_data_rnc_1group_M9.csv"),
    row.names = FALSE
  )
  write.csv(
    rbind(
    ...
    file = file.path(SYNTHETIC_DATA_DIR, "mock_data_rnc_2groups_M9.csv"),
    row.names = FALSE
  )
}
...
makeSpecimensWithinSubjects <- function(n_subjects_per_group = 25, seed = NULL)
{
  ...
  write.csv(
    data_holder,
    file = file.path(SYNTHETIC_DATA_DIR, "mock_data_rnc_2x2groups_M9.csv"),
    row.names = FALSE
  )
}
