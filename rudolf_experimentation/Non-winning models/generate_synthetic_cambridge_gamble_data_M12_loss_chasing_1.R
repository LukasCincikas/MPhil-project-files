...
utilityFunction4 <- function(x, rho, points_ratio)
{
  # Utility function log(1+x*rho)

  return(log(1 + x * (points_ratio ^ rho)))
}
  ...
  debug = TRUE)
{
  ...
  utility_after_loss <- utilityFunction4(y_capital_after_loss, rho, points_ratio)
  ...
  for (block in 1 : N_BLOCKS) {
    ...
    while (trial_within_block <= MAX_TRIALS_PER_BLOCK) {
      ...
      if (trial_within_block == 1) {
        ...
        points_ratio <- start_points / starting_points[trial-1]
      }
...
DEFAULT_RHO <- 0  # flat utility function; (roughly) [Romeu2020], fig. 3, controls
...
makeSpecimensWithinSubjects <- function(n_subjects_per_group = 25, seed = NULL)
{
  ...
  write.csv(
    data_holder,
    file = file.path(SYNTHETIC_DATA_DIR, "mock_data_rnc_2x2groups_M12_loss_chasing_1.csv"),
    row.names = FALSE
  )
}
