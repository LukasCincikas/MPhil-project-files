    ...
    for (t in 1:N_TRIALS) {
        ...
        if (t == 1) {
          p_subject_chooses_red = pChooseRedNoAlpha(proportion_red, red_bias);
        } else if (block_number[t] != block_number[t-1]) {
          p_subject_chooses_red = pChooseRedNoAlpha(proportion_red, red_bias);
        } else if ((chose_red[t-1] == 1 && subject_won[t-1] == 1) || (chose_red[t-1] == 0 && subject_won[t-1] == 0)) {
          p_subject_chooses_red = pChooseRedLeftWonNoAlpha(proportion_red, red_bias, phi);
        } else {
          p_subject_chooses_red = pChooseRedLeftLostNoAlpha(proportion_red, red_bias, phi);
        }
        ...
        penalized_expected_utility = getPenalizedExpectedUtilityLoss3C(
