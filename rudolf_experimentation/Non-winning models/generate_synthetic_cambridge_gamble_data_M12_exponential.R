  ...
  debug = TRUE)
{
  ...
  penalty_for_waiting <- exp(-1*beta*(bet_position - 1))
  
  penalized_expected_utility <- NULL
  
  for(i in 1:N_BET_OPTIONS) {
    penalized_expected_utility[i] <- expected_utility[i] * penalty_for_waiting[i]
  }
...
makeSpecimensWithinSubjects <- function(n_subjects_per_group = 25, seed = NULL)
{
  ...
  write.csv(
    data_holder,
    file = file.path(SYNTHETIC_DATA_DIR, "mock_data_rnc_2x2groups_M12_exponential.csv"),
    row.names = FALSE
  )
}
