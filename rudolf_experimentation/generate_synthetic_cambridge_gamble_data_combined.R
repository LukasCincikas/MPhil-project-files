...
# - further edited and modified by Lukas Cincikas, 2021 to accommodate the winning model
  ...
  debug = TRUE)
{
  ...
  p_loss <- 1 - p_win
  expected_utility <- (p_win ^ zeta) * utility_after_win * points_ratio ^ theta + p_loss * utility_after_loss
  # ... [Romeu 2020], eq. 6
...
makeSingleRun <- function(alpha, red_bias, gamma, rho, beta, theta, zeta)
{
  ...
  for (block in 1 : N_BLOCKS) {
    ...
    while (trial_within_block <= MAX_TRIALS_PER_BLOCK) {
      ...
      if (trial_within_block == 1) {
        ...
        points_ratio <- starting_points[trial - 1] / starting_points[trial]
      }
...
DEFAULT_ALPHA <- 7.58  # (roughly) [Romeu2020], fig. 3, controls
DEFAULT_RED_BIAS <- 0.4067  # no bias; controls were ~0.48 in [Romeu2020], fig. 3
DEFAULT_GAMMA <- 22.53  # (roughly) [Romeu2020], fig. 3, controls
DEFAULT_RHO <- 3.05  # flat utility function; (roughly) [Romeu2020], fig. 3, controls
DEFAULT_BETA <- 0.0759  # (roughly) [Romeu2020], fig. 3, controls
DEFAULT_THETA <- 0.21
DEFAULT_ZETA <- 0.106
...
makeSpecimensWithinSubjects <- function(n_subjects_per_group = 28, seed = NULL)
{
  # These are just picked at random:
  secondary_alpha <- 2.52
  secondary_red_bias <- 0.497
  secondary_gamma <- 10.1
  secondary_rho <- 0.422
  secondary_beta <- 0.139
  secondary_theta <- 0.625
  secondary_zeta <- 0.899
  
  #For within-subject design populations, these should be used:
  drug_primary_alpha <- 0.51
  drug_primary_red_bias <- 0.00876
  drug_primary_gamma <- 6.27
  drug_primary_rho <- 0.52
  drug_primary_beta <- -0.0167
  drug_primary_theta <- 0.0053
  drug_primary_zeta <- 0.1125
  
  drug_secondary_alpha <- -0.2
  drug_secondary_red_bias <- -0.039
  drug_secondary_gamma <- -1.29
  drug_secondary_rho <- 0.376
  drug_secondary_beta <- -0.002
  drug_secondary_theta <- 0.074
  drug_secondary_zeta <- -0.081
  ...
  for(q in 1:n_subjects_per_group)
  {
    data_holder <- rbind(
      ...
      makeWithinSubjectOneInstance(
        ...
        theta = (DEFAULT_THETA + drug_primary_theta),
        zeta = (DEFAULT_ZETA + drug_primary_zeta)
      )
    )
  ...
  for(q in 1:n_subjects_per_group)
  {
    data_holder <- rbind(
      ...
      makeWithinSubjectOneInstance(
        ...
        theta = (secondary_theta + drug_secondary_theta),
        zeta = (secondary_zeta + drug_secondary_zeta)
      )
    )
  ...
  write.csv(
    data_holder,
    file = file.path(SYNTHETIC_DATA_DIR, "mock_validation_data.csv"),
    row.names = FALSE
  )
}
...
makeSpecimensWithinSubjects(seed=5555)
