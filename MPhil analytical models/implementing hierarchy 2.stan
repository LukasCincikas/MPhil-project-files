//Need to implement hierarchy as per Rudolf's suggestions
data {
...
parameters {
  ...
  real alpha_subject_t[subjects/2];
  real<lower=0> gamma_SD;
  real gamma_group_t[4];
  real gamma_subject_t[subjects/2];
}
...
transformed parameters {
  ...
  for (t in 1:(subjects/2)) {
    alpha_subject[t] = exp(alpha_subject_t[t]);
    gamma_subject[t] = exp(gamma_subject_t[t]);
  }
  for (u in 1:4) {
